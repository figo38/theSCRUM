<?php
	class Sprint extends PBObject {
		...
		protected $readSqlStatement = 'SELECT spr_id id, spr_nb sprintnb, pro_id projectid, spr_goal goal, DATE_FORMAT( spr_start_date, \'%Y-%m-%d\' ) startdate, DATE_FORMAT( spr_end_date, \'%Y-%m-%d\' ) enddate, spr_nr_days nrdays, spr_nr_hours_per_day nbhours, spr_closed closed, spr_unit unit, spr_copied_from_previous copiedfromprevious, spr_retro_1, spr_retro_2 FROM sprint WHERE spr_id=?';
		...
		public function copyTasksFromPrevious() {
			...
			if ($nb > 1) {
				...
				$sth = $DB->prepare('SELECT tsk_title title, sto_id, IF(tsk_reestim - tsk_worked < 0, 0, tsk_reestim - tsk_worked) estim, tsk_status, tsk_prio FROM task WHERE tsk_status in (0,1) AND spr_id=?');
