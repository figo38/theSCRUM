<?php
	class Project extends PBObject {
		...
		protected $readSqlStatement = 'SELECT pro_id id, pro_name name, pro_velocity velocity, pro_goal goal, pro_has_sprints hassprints, pro_unit unit FROM project WHERE pro_id=?';
		public static $realSqlSelectStatement = 'SELECT pro_id id, pro_name name, pro_velocity velocity, pro_goal goal, pro_has_sprints hassprints, pro_unit unit FROM project ';
		...
		public static function getLatestSprints() {
			return Helpers::execute('SELECT p.pro_id, pro_name projectname, spr_nb sprintnb, spr_goal sprintgoal, DATE_FORMAT( spr_start_date, \'' . DATE_FORMAT . '\' ) startdate, DATE_FORMAT( spr_end_date, \'' . DATE_FORMAT . '\' ) enddate FROM project p, sprint s WHERE p.pro_id = s.pro_id AND spr_start_date <= NOW() AND spr_end_date >= NOW() ORDER BY pro_name');
		}
